...
def hit_point_sum(unit_list):
    ...
    return hp_sum
def cheat(units, extra_attack_power):
    for unit in units:
        if(unit.type == 'E'):
            unit.attack_power += extra_attack_power
    return units
class Unit():
        ...
        for unit in units:
            ...
            #1. Identify possible targets
                ...
                if(len(targets) == 0):
                    ...
                    , '#goblins:', no_goblins)
                    no_elves = sum(u.type == 'E' for u in units)
                    no_goblins = sum(u.type == 'G' and u.alive == True for u in units)
                    no_elves_alive = sum(u.type == 'E' and u.alive == True for u in units)
                    if(no_goblins == 0 and no_elves == no_elves_alive):
                        elf_lose = False
                        print('The elves did it! With attack power ', str(3+extra_help))
